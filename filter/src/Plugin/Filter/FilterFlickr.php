...
/**
 * Provides a filter to add Flickr content.
 *
 * @Filter(
 *   id = "filter_flickr",
 *   title = @Translation("Flickr filter"),
 *   type = Drupal\filter\Plugin\FilterInterface::TYPE_MARKUP_LANGUAGE,
 *   settings = {}
 * )
 */
class FilterFlickr extends FilterBase {
  ...
  public function process($text, $langcode) {
    ...
    $pattern = '/\[flickr\|entity_type=(\w+)\|entity_id=(\d+)\|view_mode=(\w+)\]/s';
    $text = preg_replace_callback($pattern, array($this, 'flickr_process_callback'), $text);
  ...
  public function tips($long = FALSE) {
    $tip = t('Converts content tokens in the format %pattern into the appropriate rendered content and makes all paths absolute. Use the "Insert Site Content" widget below to generate tokens.',
      array('%pattern' => '[flickr|entity_type=node|entity_id=1|view_mode=teaser]')
    );
